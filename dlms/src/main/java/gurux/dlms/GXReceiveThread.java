...
class GXReceiveThread extends Thread {
     ...
     *
     */
    GXReceiveThread(final GXSerial parent) {
        parentMedia = parent;
        hcomPort = parent.getPort();
    }
    ...
    private void handleReceivedData(final byte[] buffer) {
        ...
        if (parentMedia.getIsSynchronous()) {
            ...
            ReceiveEventArgs e =
                    new ReceiveEventArgs(data, parentMedia.getName());
            parentMedia.notifyReceived(e);
        }
    }

    private byte[] dataToRead;
